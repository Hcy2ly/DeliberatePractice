...
1. 每日算法：字符串相乘
   ...
   解答：
      竖式相乘：计算 num1 依次乘上 num2 的每一位的和，把得到的所有和按对应的位置累加在一起，就可以得到 num1 * num2 的结果。[multiply.js]
...
1. 浏览器原生css 替代 js的一些优秀片段
   ...
   css原生在以下场景的处理方式：[节省js代码的一些css代码片段]
      1. 限制显示的行数
...
4. monorepo概念：https://www.perforce.com/blog/vcs/what-monorepo
...
5. 推荐使用pnpm： https://juejin.cn/post/6932046455733485575#comment
   pnpm也是一个包管理器，相较于npm/yarn它的优势在于？
    1. 包安装速度极快
    2. 磁盘空间利用非常高效
    3. 安装简单 npm i -g pnpm

   yarn PnP（Plug’n’Play）安装模式：直接去掉 node_modules，将依赖包内容写在磁盘，节省了 node 文件 I/O 的开销，这样也能提升安装速度。（原理解析：https://loveky.github.io/2019/02/11/yarn-pnp/）
